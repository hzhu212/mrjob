...
if __name__ == '__main__':
    WordCount().run(runner='local', input='data.txt')
```
...
2. 由于要向输出文件中写入数据，`reducer` 与 `mapper` 的过程刚好相反，你可以把 `key` 置为 `None` 来避免把 `key` 写入输出文件。如果 `key` 非 `None`，那么它将与 `value` 通过 `\t` 拼接在一起，然后写入输出文件。这种情况下，即使 `value` 是序列类型也会得到预期的结果，例如：`yield 'a', [1, 2, 3]` 会得到预期的 `a\t1\t2\t3`，而不是 `a\t[1, 2, 3]`。
